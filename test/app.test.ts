...
const fixturePath = path.join(__dirname, "../test/fixtures/existing-project");
...
describe("generator-ccc:app", function () {
  ...
  describe("fresh project", function () {
    before(async function () {
      await helpers.run(generatorPath).withOptions({ skipInstall: true });
    });
    ...
    it("writes default vscode settings", function () {
      assert.fileContent(".vscode/settings.json", /"editor\.formatOnSave": true/);
      assert.fileContent(
        ".vscode/settings.json",
  ...
  describe("existing gitignore", function () {
    it("does not duplicate gitignore entries when defaults already exist", async function () {
      ...
      const nodeModulesCount = contents.split("node_modules/").length - 1;
